...
fn build_nsis_app_installer(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        crate::sign::try_sign(&app_exe_source, config)?;
    }
    ...
    nsis_cmd
        .arg(match config.log_level.unwrap_or_default() {
            LogLevel::Error => "-V1",
            LogLevel::Warn => "-V2",
            LogLevel::Info => "-V3",
            LogLevel::Debug => "-V3",
...
fn generate_resource_data(config: &Config) -> ResourcesMap {
    ...
    if let Some(resources) = config.resources() {
        for resource in resources {
            ...
            if added_resources.contains(&resource.src) {
                continue;
            }
            added_resources.push(resource.src.clone());
